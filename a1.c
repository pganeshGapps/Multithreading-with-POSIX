...
int firstIterFlag=1; // denoting first ever iteration over the entire program
int main()
{
 ...
 while(1){
    ...
    if(firstIterFlag == 1){ threads = (pthread_t *) malloc(sizeof(pthread_t) * threadCount); }
    if(threadCount == prevThreadCount){}
    else if(threadCount > prevThreadCount){ threads= (pthread_t *) realloc(threads,(threadCount-prevThreadCount)); }
    else if(threadCount < prevThreadCount){ threads= (pthread_t *) realloc(threads,(prevThreadCount-threadCount)); }         
        
    prevThreadCount=threadCount;//update
	...
	/*
     ...
     int choice=0;
     printf("Continue ?(Enter 0 for No) : ");
     scanf("%d",&choice);
     if(choice==0){break;}
     else{firstIterFlag=0;}
 }
    return 0;
}
